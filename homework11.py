...
# helper class to hold optimizer name and params
class Optimizer:
...
def build_sgd(params):
    return Optimizer("SGD", params)


def build_adam(params):
    return Optimizer("Adam", params)


def build_adagrad(params):
    return Optimizer("Adagrad", params)


def build_rmsprop(params):
    return Optimizer("RMSprop", params)
...
class OptimizerMLP:
    def __init__(self, max_epochs, units_per_layer, batch_size=20, step_size=0.01):
        ...
        self.opt = build_sgd({"momentum": 0.5})

    def __build_optimizer(self):
        if self.opt.name == "SGD":
            self.optimizer = torch.optim.SGD(self.nn.parameters(), lr=self.step_size, **self.opt.params)
        elif self.opt.name == "Adam":
            self.optimizer = torch.optim.Adam(self.nn.parameters(), lr=self.step_size, **self.opt.params)
        elif self.opt.name == "Adagrad":
            self.optimizer = torch.optim.Adagrad(self.nn.parameters(), lr=self.step_size, **self.opt.params)
        elif self.opt.name == "RMSprop":
            self.optimizer = torch.optim.RMSprop(self.nn.parameters(), lr=self.step_size, **self.opt.params)
        else:
...
def full_run(params_of_interest):
    ...
    for data_set, data in data_dict.items():
        ...
        max_epochs = [50, 100, 400, 600, 900, 1100]
        ...
        for momentum in 0.1, 0.5:
            optimizers.append(build_sgd({"momentum": momentum}))
        for beta1 in 0.85, 0.95:
            for beta2 in 0.999,:# 0.999, 0.9999:
                optimizers.append(build_adam({"betas": (beta1, beta2)}))

        hyperparams = combine_hyperparams(
            ["max_epochs", "batch_size", "opt"],
            [max_epochs, batch_sizes, optimizers]
        )

        mlp = OptimizerMLP(max_epochs=250, units_per_layer=mlp_layers, batch_size=batch_size, step_size=step_size)
        ...
        time_taken = time.time() - time_start
        print("Done running OptimizerMLP in", time_taken, "seconds.")
    ...
    for param_of_interest in params_of_interest:
        ...
        param_view = loss_df.groupby(["optname", "train_set", "data_set", param_of_interest])["loss"].mean().reset_index()
        gg = p9.ggplot() + \
            p9.geom_line(
                p9.aes(
                    x=param_of_interest,
                ...
                data=param_view) + \
            p9.facet_grid("data_set ~ optname") + \
            p9.theme(figure_size=(24, 16))
        gg.save("homework11_" + param_of_interest + "_loss.png")
    pass
...
def experiments():
    ...
    for data_set, (input_data, output_labels) in data_dict.items():
        ...
        for fold_id, indices in enumerate(k_fold_split):
            ...
            torch_mlp = OptimizerMLP(max_epochs=250, units_per_layer=mlp_layers, batch_size=batch_size,
                                     step_size=step_size)
    ...
    gg.save("homework11_comparison.png")


full_run(["max_epochs", "batch_size"])
experiments()
