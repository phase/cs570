#
# CS 570 Spring 2022
# Homework 7 / week 8 - Jadon Fowler
#
import os.path
import random
import time
...
# Using Torch instead of our own implementation
class TorchMLP:
    def __init__(self, max_epochs, units_per_layer, batch_size=20, step_size=0.01):
        self.max_epochs = max_epochs
        self.batch_size = batch_size
...
class TorchMLPCV:
    def __init__(self, estimator, param_grid={'n_neighbors': [x for x in range(1, 10)]}, cv=4):
        self.fit_results = None
        self.best_params_ = None
        self.cv = cv
        self.estimator = estimator
...
def full_run():
    ...
    for data_set, data in data_dict.items():
        ...
        if data_set == "spam":
            ...
            step_size = 0.005
            batch_size = 10
        max_epochs = [50, 100, 250, 400, 500, 600, 750, 900, 1000]
        ...
        mlp = TorchMLP(max_epochs=250, units_per_layer=mlp_layers, batch_size=batch_size, step_size=step_size)
        automlp_cv = TorchMLPCV(mlp, cv=6, param_grid={'max_epochs': max_epochs})
        ...
        print("Done running TorchMLP in", time_taken, "seconds.")
    ...
    gg.save("homework7_epoch_min_loss.png")
    gg = p9.ggplot() + \
         p9.geom_line(
             p9.aes(
                 x="max_epochs",
             ...
             data=epoch_means_df) + \
         p9.facet_grid(". ~ data_set")
    gg.save("homework7_epoch_mean_loss.png")
...
def experiments():
    ...
    for data_set, (input_data, output_labels) in data_dict.items():
        ...
        for fold_id, indices in enumerate(k_fold_split):
            ...
            time_start = time.time()
            print("Running TorchMLP...")
            torch_mlp = TorchMLP(max_epochs=250, units_per_layer=mlp_layers, batch_size=batch_size, step_size=step_size)
            torch_mlp_cv = TorchMLPCV(torch_mlp, cv=6, param_grid={'max_epochs': max_epochs})
